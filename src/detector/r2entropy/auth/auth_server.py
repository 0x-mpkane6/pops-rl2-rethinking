...
IPID_MODE_FILE = os.getenv("IPID_MODE_FILE", "/app/ipid_mode")
IPID_MODE = os.getenv("IPID_MODE", "random").strip().lower()
...
def pick_ipid() -> int:
    """Allow E5 to pin FRAG1 IPID without recreating the auth container."""
    text = IPID_MODE
    try:
        with open(IPID_MODE_FILE, "r", encoding="utf-8") as handle:
